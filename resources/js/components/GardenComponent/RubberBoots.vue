    ...
    <div style="height:0px">
        <svg  @click="getCircle(dynamicIndexValue, $event)" v-for="(items , index) in dataloop" :key="index" version="1.1" baseProfile="basic"  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" data-toggle="modal" :data-target="'#myModal'+dynamicIndexValue"  :style="{transform:'rotate('+sliderValue+'deg) scale('+scaleValue+') scaleX('+flipElement+')'}">
                <g style="transform: scale(0.5);">
                   <path style="fill:#FAA33A; transform:scale(0.2)" :class="ValueId+'_color4'" d="M151.9,672.2c-0.5-1.1-1-2.2-1.5-3.2c-3.6-6.3,0.2-11.8,2-17.4c8.6-13.9,21.5-20.2,37.4-19.5
...
<path style="fill:#E29435; transform:scale(0.2)" :class="ValueId+'_color2'" d="M308.4,133.4c1.9,20.4,3.9,40.8,5.7,61.3c1.2,13.6,2.2,27.3,3.4,40.9c1.9,21.2,4,42.4,5.8,63.6
...
<path style="fill:#130A04; transform:scale(0.2)" :class="ValueId+'_color3'" d="M527.2,672.4c5.9-0.1,11.8,0.4,17.5-0.5c5.2-0.8,6,1.1,6,5.6c-0.1,14.8,0.2,29.5,0.3,44.3
...
<path style="fill:#140B05; transform:scale(0.2)" :class="ValueId+'_color3'" d="M55.5,627.7c30.8,15.7,63.5,23.7,98,24.9c-1.3,5.4-3.2,10.7-3.1,16.4c-5-0.2-11.7-3.8-11.8,5.9
...
<path style="fill:#CB0909; transform:scale(0.2)" d="M522.1,723.7c8.8-0.6,17.6-0.6,26.3,0.2C539.6,723.8,530.8,723.8,522.1,723.7z"/>
<path style="fill:#EDC33A; transform:scale(0.2)" :class="ValueId+'_color1'" d="M458.2,81.3c-10.5,7.6-22.9,8.9-35.2,10.7c-25.3,3.8-50.8,5.3-76.4,6.5c-16.5,0.8-33.1,1.6-49.5,0.8
                        ...
                        <div class="tab-content">
                            <div :class="{'tab-pane':true, active:ActivePalette=='active'}" id="palette">
                                <div class="bulldog_svg" :id="svgName+dynamicIndex" v-if="this.$store.state.dynamicName ===svgName+dynamicIndex">
                                    <h2>Select Color</h2>
                                    <button :style="{background:'rgba('+getterRubberBootsBg1+')'}" @click="ShowElement(getterRubberBootsBg1)" :class="this.ValueId+'_color1btn'"></button>
                                    <button :style="{backgroundColor:'rgba('+getterRubberBootsBg2+')'}" @click="ShowElement1(getterRubberBootsBg2)" :class="this.ValueId+'_color2btn'"></button>
                                    <button :style="{backgroundColor:'rgba('+getterRubberBootsBg3+')'}" @click="ShowElement2(getterRubberBootsBg3)" :class="this.ValueId+'_color3btn'"></button>

                                    <button :style="{backgroundColor:'rgba('+getterRubberBootsBg4+')'}" @click="ShowElement3(getterRubberBootsBg4)" :class="this.ValueId+'_color4btn'"></button>

                                    <!-- <button :style="{backgroundColor:'rgba('+getterRubberBootsBg5+')'}" @click="ShowElement4(getterRubberBootsBg5)" :class="this.ValueId+'_color5btn'"></button> -->
                                </div>
...
<script>
    ...
    export default {
        ...
        if(this.$store.state.RandomClicked == true){
            var randomNumber = RubberBoots[this.$store.state.randomIndexElement].left
            var randomNumberTop  =  RubberBoots[this.$store.state.randomIndexElement].top
            if(this.$store.state.randomFirstSvg == 'RubberBoots'){
        ...
        computed: {
            ...mapState([
                ...
                getterRubberBootsBg1: {
                    get() {
                        // console.log(this.$store.state.SvgComponent[this.$store.state.dynamicIndex][0])
                        if (this.$store.state.SvgComponent[this.$store.state.dynamicIndex][0].name === 'RubberBoots') {
